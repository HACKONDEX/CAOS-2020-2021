...
char* DICTIONARY_PATH_CONST = NULL;

void execute_file(int connection_fd, char* file_name) {
    char file_path_full[MAXSIZE];
    strcpy(file_path_full, DICTIONARY_PATH_CONST);
    int tmp_length = strlen(file_path_full);
    if(file_path_full[tmp_length - 1] != '/') {
        file_path_full[tmp_length] = '/';
        file_path_full[tmp_length + 1] = '\0';
    }
    strcat(file_path_full, file_name);

    pid_t pid = fork();
    if(pid == 0) {
        dup2(connection_fd, 1);
        execlp(file_path_full, file_path_full, NULL);
        return;
    }
...
void process_client(int directory_fd, int connection_fd) {
    ...
    // If is not executable
    if(faccessat(directory_fd, file_name, X_OK, 0) != 0) {
    ...
    // Is executable
    execute_file(connection_fd, file_name);
}
...
int main(int argc, char *argv[]) {
    ...
    DICTIONARY_PATH_CONST = argv[2];

    pipe(stop_pipe_fds);
    set_handlers();

    launch_server(PORT, DICTIONARY_PATH_CONST, stop_pipe_fds[0]);
